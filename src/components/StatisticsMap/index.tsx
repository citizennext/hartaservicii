...
const query = graphql`
  query {
    hasura {
      alba: providers_aggregate(where: { district: { _eq: "Alba" } }) {
        aggregate {
          count
        }
      }
      arad: providers_aggregate(where: { district: { _eq: "Arad" } }) {
        aggregate {
          count
        }
      }
      arges: providers_aggregate(where: { district: { _eq: "Argeș" } }) {
        aggregate {
          count
        }
      }
      bacau: providers_aggregate(where: { district: { _eq: "Bacău" } }) {
        aggregate {
          count
        }
      }
      bihor: providers_aggregate(where: { district: { _eq: "Bihor" } }) {
        aggregate {
          count
        }
      }
      bistritanasaud: providers_aggregate(where: { district: { _eq: "Bistrița Năsăud" } }) {
        aggregate {
          count
        }
      }
      botosani: providers_aggregate(where: { district: { _eq: "Botoșani" } }) {
        aggregate {
          count
        }
      }
      braila: providers_aggregate(where: { district: { _eq: "Brăila" } }) {
        aggregate {
          count
        }
      }
      brasov: providers_aggregate(where: { district: { _eq: "Brașov" } }) {
        aggregate {
          count
        }
      }
      ...
      calarasi: providers_aggregate(where: { district: { _eq: "Călărași" } }) {
        aggregate {
          count
        }
      }
      carasseverin: providers_aggregate(where: { district: { _eq: "Caraș Severin" } }) {
        aggregate {
          count
        }
      }
      cluj: providers_aggregate(where: { district: { _eq: "Cluj" } }) {
        aggregate {
          count
        }
      }
      constanta: providers_aggregate(where: { district: { _eq: "Constanța" } }) {
        aggregate {
          count
        }
      }
      covasna: providers_aggregate(where: { district: { _eq: "Covasna" } }) {
        aggregate {
          count
        }
      }
      dambovita: providers_aggregate(where: { district: { _eq: "Dâmbovița" } }) {
        aggregate {
          count
        }
      }
      dolj: providers_aggregate(where: { district: { _eq: "Dolj" } }) {
        aggregate {
          count
        }
      }
      galati: providers_aggregate(where: { district: { _eq: "Galați" } }) {
        aggregate {
          count
        }
      }
      giurgiu: providers_aggregate(where: { district: { _eq: "Giurgiu" } }) {
        aggregate {
          count
        }
      }
      gorj: providers_aggregate(where: { district: { _eq: "Gorj" } }) {
        aggregate {
          count
        }
      }
      harghita: providers_aggregate(where: { district: { _eq: "Harghita" } }) {
        aggregate {
          count
        }
      }
      hunedoara: providers_aggregate(where: { district: { _eq: "Hunedoara" } }) {
        aggregate {
          count
        }
      }
      ialomita: providers_aggregate(where: { district: { _eq: "Ialomița" } }) {
        aggregate {
          count
        }
      }
      ...
      maramures: providers_aggregate(where: { district: { _eq: "Maramureș" } }) {
        aggregate {
          count
        }
      }
      mehedinti: providers_aggregate(where: { district: { _eq: "Mehedinți" } }) {
        aggregate {
          count
        }
      }
      mures: providers_aggregate(where: { district: { _eq: "Mureș" } }) {
        aggregate {
          count
        }
      }
      neamt: providers_aggregate(where: { district: { _eq: "Neamț" } }) {
        aggregate {
          count
        }
      }
      olt: providers_aggregate(where: { district: { _eq: "Olt" } }) {
        aggregate {
          count
        }
      }
      prahova: providers_aggregate(where: { district: { _eq: "Prahova" } }) {
        aggregate {
          count
        }
      }
      salaj: providers_aggregate(where: { district: { _eq: "Sălaj" } }) {
        aggregate {
          count
        }
      }
      satumare: providers_aggregate(where: { district: { _eq: "Satu Mare" } }) {
        aggregate {
          count
        }
      }
      sibiu: providers_aggregate(where: { district: { _eq: "Sibiu" } }) {
        aggregate {
          count
        }
      }
      suceava: providers_aggregate(where: { district: { _eq: "Suceava" } }) {
        aggregate {
          count
        }
      }
      teleorman: providers_aggregate(where: { district: { _eq: "Teleorman" } }) {
        aggregate {
          count
        }
      }
      timis: providers_aggregate(where: { district: { _eq: "Timiș" } }) {
        aggregate {
          count
        }
      }
      tulcea: providers_aggregate(where: { district: { _eq: "Tulcea" } }) {
        aggregate {
          count
        }
      }
      valcea: providers_aggregate(where: { district: { _eq: "Vâlcea" } }) {
        aggregate {
          count
        }
      }
      vaslui: providers_aggregate(where: { district: { _eq: "Vaslui" } }) {
        aggregate {
          count
        }
      }
      vrancea: providers_aggregate(where: { district: { _eq: "Vrancea" } }) {
        aggregate {
          count
        }
      }
...
function StatisticsMap(props: Props) {
  ...
  return (
    <StaticQuery
      query={query}
      render={data => {
        const districtAmounts = Object.values(data.hasura).map((d: any) => d.aggregate.count);
        const maxAmount = Math.max(...districtAmounts);
        const minAmount = Math.min(...districtAmounts);
        ...
        return (
          <div className={props.classStatisticsMap}>
            <svg width={sqSizeWidth} height={sqSizeHeight} viewBox={viewBox}>
              ...
              {Object.entries(data.hasura).map((socialService: any, index) => {
                return (
                  <path
